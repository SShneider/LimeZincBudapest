...
async function fetchGroupPredictions(requestIn, apiKey, BoX, existingScores =''){
  ...
  function setExistingScores(allMatches, doneMatches){
    ...
    for(let i = 0; i<allMatches.length; i++){
      ...
      potentialKeyTwo = plaLower+plbLower
      console.log(potentialKeyTwo, potentialKeyOne)
      if(doneMatches[potentialKeyOne]) actualKey = potentialKeyOne
      else if(doneMatches[potentialKeyTwo]){
        actualKey = potentialKeyTwo
      }
      if(actualKey.length){
        existingScores.push(allMatches[i].identifier+'_'+doneMatches[actualKey][plaLower])
        existingScores.push(allMatches[i].identifier+'_'+doneMatches[actualKey][plbLower])
      }
    }
